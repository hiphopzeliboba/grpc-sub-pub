...
- `Паттерн` DI контейнер в ```/app```
- Абстракция через интерфейсы
- Изменение параметров сервиса в конфиге (.env)
- `Паттерн` Graceful shutdown (корректное завершение по Ctrl+C) в ```/Closer```
...
- ps. Проект не лишен нюансов и косяков.  Старался показать, что знаком с технологиями, паттернами и практиками, применяемыми в разарботке микросервисов как в бигтехах. Постарался соблюсти чистую архитектуру)п